...
export class Model {
  ...
  @action public init(width: number, height: number): void {
    this.board = [];
    this.snake = [];
    this.direction = Direction.right;
    this.finished = false;
  ...
  @action public initSnake(x: number, y: number, length: number): void {
    this.snake = [];
  ...
  @action public createRandom(count: number, content: Field): void {
